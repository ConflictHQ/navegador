...
class TestFunctionCommand:
    ...
    def test_function_with_file_option(self):
        ...
        with (
            ...
            MockCL.return_value.load_function.assert_called_with("foo", file_path="bar.py", depth=2)
            assert result.exit_code == 0
...
class TestGetStoreCustomPath:
    """_get_store resolves storage and reports which layer decided it."""

    def test_custom_path_selects_that_embedded_graph(self):
        from navegador.cli.commands import _open_store

        with patch("navegador.config.open_store", return_value=_mock_store()):
            _, config = _open_store("/custom/path.db")
        assert config.backend == "embedded"
        assert config.db_path == "/custom/path.db"
        assert config.source == "--db"

    def test_default_path_falls_through_to_configuration(self, tmp_path, monkeypatch):
        """The --db default is a sentinel, not a choice — config decides instead."""
        from navegador.cli.commands import _open_store
        from navegador.config import DEFAULT_DB_PATH

        monkeypatch.delenv("NAVEGADOR_DB", raising=False)
        monkeypatch.setenv("NAVEGADOR_REDIS_URL", "redis://configured:6379")
        with patch("navegador.config.open_store", return_value=_mock_store()):
            _, config = _open_store(DEFAULT_DB_PATH)
        assert config.backend == "redis"
        assert config.redis_url == "redis://configured:6379"

    def test_resolution_failure_becomes_a_click_error(self):
        """A bad backend must not reach the user as a redis-py traceback (#170)."""
        import click

        from navegador.cli.commands import _open_store
        from navegador.config import StorageResolutionError

        with patch("navegador.config.open_store", side_effect=StorageResolutionError("boom")):
            with pytest.raises(click.ClickException):
                _open_store("/custom/path.db")
    ...
    def test_export_success(self):
        ...
        with runner.isolated_filesystem():
            with (
                patch("navegador.cli.commands._get_store", return_value=_mock_store()),
                patch(
                    "navegador.graph.export.export_graph", return_value={"nodes": 10, "edges": 5}
                ),
            ):
    ...
    def test_import_success(self):
        ...
        with runner.isolated_filesystem():
            ...
            with (
                patch("navegador.cli.commands._get_store", return_value=_mock_store()),
                patch(
                    "navegador.graph.export.import_graph", return_value={"nodes": 10, "edges": 5}
                ),
            ):
    ...
    def test_import_no_clear(self):
        ...
        with runner.isolated_filesystem():
            ...
            with (
                patch("navegador.cli.commands._get_store", return_value=_mock_store()),
                patch(
                    "navegador.graph.export.import_graph", return_value={"nodes": 0, "edges": 0}
                ) as mock_imp,
            ):
